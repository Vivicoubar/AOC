...
func main() {
	...
	for scanner.Scan() {
		...
		if x1 == x2 {
			cloudMap = processVerticalLine(x1, y1, y2, cloudMap)
		} else if y1 == y2 {
			cloudMap = processHorizontalLine(x1, x2, y1, cloudMap)
		} else {
...
func processHorizontalLine(x1 int, x2 int, y1 int, cloudMap [][]int) [][]int {
	if x1 < x2 {
		for i := x1; i <= x2; i++ {
			cloudMap[y1][i] += 1
		}
...
func processVerticalLine(x1 int, y1 int, y2 int, cloudMap [][]int) [][]int {
	if y1 < y2 {
		for i := y1; i <= y2; i++ {
			cloudMap[i][x1] += 1
		}
...
func processDiagonal(x1 int, y1 int, x2 int, y2 int, cloudMap [][]int) [][]int {
	if y1 < y2 {
		for j := y1; j <= y2; j++ {
			if x1 < x2 {
				for i := x1; i <= x2; i++ {
